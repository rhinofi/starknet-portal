import { CoreCard, Input, Select, Spacing, Text, Title, TokenInput } from '@deversifi/dvf-shared-ui'
...
import { BridgeFundsPendingTransfers } from './BridgeFundsPendingTransfers'
...
export const BridgeFunds = () => {
  ...
  const { token, amount } = useMemo(() => formRef?.current?.values || {}, [formRef?.current?.values]) as FormValues
    ...
    <Page>
      <ContentWrapper>
        <TextWrapper direction='column'>
          ...
          <Spacing size='24' />
          <Text>
            When you provide liquidity, your contribution helps fill the pool for this pair of tokens. The liquidity in these pools enables an automated market maker to adjust the value of each token, so traders can swap one for another instantly. As a liquidity provider, you get two sets of rewards: you’ll receive a share of the trading fees as well as DVF rewards.
          </Text>
          <BridgeFundsPendingTransfers />
          {/* <BridgeFundsSteps token={token} amount={amount} isDeposit={isDeposit} /> */}
        </TextWrapper>
        <CoreCard>
          <Formik
            initialValues={{
            ...
            {({ values, setFieldValue, isValid, touched }) => (
              <>
                ...
                <BridgeFundsWidgetCTA
                  ...
                  resetAmount={() => setFieldValue('amount', '')}
                />
              </>
            )}
          </Formik>
